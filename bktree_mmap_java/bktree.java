...
// constructors 
    ...
    Node() {
        int i;
        //initially all characters have been initialized to ' '
        for (i = 0; i < 30; i++)
            word[i] = ' ';
            // initializing next[i] = 0 
        for (i = 0; i < 2 * 30; i++)
...
// filling base cases 
    ...
    //getting node of tree with the given index
    ...
    // putting node into the tree with the given index
    ...
    // getting root node, index can only be zero
    ...
    // putting root node, index can only be zero
...
// adds curr Node to the tree 
    ...
    public ArrayList<String> getsimilarWords(int a, String s) {
        Node rot;
        if (a == -1) {
            rot = getNodeRT(0); //first time, it gets the root node
        } else {
            rot = getNode(a); //other times it gets the node from tree with the given index a
        }
  ...
  // iterate over the string havinng tolerane 
    ...
    public static void main(String args[]) throws FileNotFoundException, IOException, InterruptedException {
        ...
        //opening mmap file for root node
        ...
        char choice;
            ...
            System.out.println("Constructing BK Tree");
            File file = new File("words.txt");
            Scanner sc1 = new Scanner(file);
            while (sc1.hasNext()) {
            String u = sc1.next();

            if ((u.charAt(0) >= 'a' && u.charAt(0) <= 'z')) {   //only words are inserted
                char x1[] = new char[30];

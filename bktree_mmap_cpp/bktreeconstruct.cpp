/* run this file only after running initbk.cpp file, it should be run only once to prepare the bk tree*/
...
// adding dict[] words on to tree 
    ...
    for(int i=0; i<sz; i++) 
    { 
        char dic[dictionary[i].length()+1];
        strcpy(dic,dictionary[i].c_str());
        Node tmp = Node(dic); 
        add(tmp); 
    }   
